...
def instanciate_nexus(raw_path, output, nxdl):
    with h5py.File(os.path.join(raw_path, output), 'w') as f:
...
def write_data_from_nomad(raw_path, output, nome, valore):
    with h5py.File(os.path.join(raw_path, output), 'a') as f:
        entry = f['entry']
        entry.create_dataset(nome, data=valore)

def write_user_from_nomad(raw_path, output, count, dati):
    with h5py.File(os.path.join(raw_path, output), 'a') as f:
        entry = f['entry']
        user = entry.create_group(f'user_{count}')
        user.attrs['NX_class'] = 'NXuser'
        for row in dati:
            if dati[row] is not None and 'm_' not in row:
                user.create_dataset(row, data=dati[row])